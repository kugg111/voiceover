...
internal class Nsnet2Processor : IDisposable
{
    ...
    public Nsnet2Processor(string modelPath)
    {
        ...
        // threaded, no-separate-thread-pool execution instead. CPU-only:
        // an earlier DirectML GPU path was removed (see git history) -
        // this model is small enough that GPU dispatch overhead routinely
        // exceeded what it saved on compute, and it added a WMI-based GPU
        // picker (System.Management) for a feature that rarely helped.
