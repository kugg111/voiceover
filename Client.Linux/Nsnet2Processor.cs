...
internal class Nsnet2Processor : IDisposable
{
    ...
    // No DirectML/GPU path on Linux (that's a Windows-only DX12 execution
    // provider) - CPU-only, matching the plan's explicit v1 scope ("drop
    // the GPU device picker for v1; CPU-only NSNet2 is already proven to
    // perform well in this app's own history").
